...
const Table = ({ allData }: ITableProps) => {
    ...
    do {
      try {
        const response = await fetch(
          `/api/projects/findAll?page=${currentPage}`
        );
        if (!response.ok) {
    ...
    if (searchTerm.trim()) {
      ...
      return allProjects.filter((project) => {
        ...
        return searchTerms.every((term) =>
          Object.values(projectData).some((value) => value.includes(term))
        );
      });
    }
  ...
  const handleSearch = useCallback(
    async (query: string) => {
      ...
      try {
        ...
        alert(
          "Hubo un error al realizar la búsqueda. Por favor, intente nuevamente."
        );
      } finally {
  ...
  const handleDelete = async (id: number) => {
    const isConfirmed = window.confirm(
      "¿Estás seguro que deseas borrar el proyecto?"
    );
    if (!isConfirmed) return;

    try {
      const res = await fetch(
        `${EndpointProjects.DELETE_PROJECT.replace(":id", id.toString())}`,
        {
          method: "DELETE",
        }
      );
      ...
      alert(
        "Hubo un error al eliminar el proyecto. Por favor, intente nuevamente."
      );
    }
  };
